# 使用方法:在根目录下 `docker build -t rcore-tutorial-v3:latest --target build .`
# --target的意思是分阶段build、该dockerfile包括3个阶段
...
FROM dockerpull.org/ubuntu:20.04 as build_qemu
...
FROM dockerpull.org/ubuntu:20.04 as build
